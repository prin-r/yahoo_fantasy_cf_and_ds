...
from logger import YahooLogger
...
class BaseOAuth(object):
    ...
    def __init__(self, oauth_version, consumer_key, consumer_secret, **kwargs):
        ...
        if self.oauth_version == "oauth1":
            ...
            service_params = {"client_id": self.consumer_key, "client_secret": self.consumer_secret}
        ...
        if (
            vars(self).get("access_token")
            and vars(self).get("access_token_secret")
            and vars(self).get("session_handle")
        ):
            if not self.token_is_valid():
                data.update(self.refresh_access_token())
        elif (
            vars(self).get("access_token")
            and vars(self).get("token_type")
            and vars(self).get("refresh_token")
        ):
            if not self.token_is_valid():
                data.update(self.refresh_access_token())
        else:
        ...
        * Get access token
        ...
        if self.oauth_version == "oauth1":
            ...
            authorize_url = self.oauth.get_authorize_url(
                redirect_uri=self.callback_uri, response_type="code"
            )
        ...
        if self.oauth_version == "oauth1":
            raw_access = self.oauth.get_raw_access_token(
                request_token, request_token_secret, params={"oauth_verifier": self.verifier}
            )
